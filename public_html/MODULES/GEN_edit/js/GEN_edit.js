...
var parsePOSTfile     = 'MODULES/GEN_edit/parsePOST.php';    // scriptul dorit pt $.post()
var parsePOSTfile4    = 'MODULES/SEO/ADMIN/SEO.php';
...
function SEOEditITEM(id)      {
    ...
    $('span.editMode')
        .append("<div class='itemSEO'>")
            .find(".itemSEO")
                .load( procesSCRIPT_file, {parsePOSTfile:parsePOSTfile4, id:id});
...
function Save_SEOEdit(id)     {
    ...
    if(!SEObut.attr('name'))   //daca nu mai exista butonul de SEO inseamna ca s-a activat SEO
    {
         ...
         title_tag        = $('span[class^=editMode]  input[name=title_tag]').val();
         title_meta       = $('span[class^=editMode]  input[name=title_meta]').val();
         description_meta = $('span[class^=editMode]  input[name=description_meta]').val();
         keywords_meta    = $('span[class^=editMode]  input[name=keywords_meta]').val();
...
function SaveEditITEM(id)     {
          ...
          val      = $('span[class^=editMode]  input[name=item_name]').val();
          itemTYPE = $('span[class^=editMode]  select[name=type]').val();
