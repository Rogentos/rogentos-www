...
class Envelope
{
    ...
    public function __construct($items) {
        ...
        foreach ($this->items as $key => $value) {
            $this->items[$key] = array('args' => $value[0], 'desc' => $value[1], 'content' => $_POST[$key]);
        }
    ...
    protected function parseArgs($items) {
        foreach($items as $key => $item) {
            ...
            switch ($n) {
                case 0:
                    $this->checked[$key] = $this->v->$rule($this->items[$key]['content']);
                    break;
                case 1:
                    $this->checked[$key] = $this->v->$rule($this->items[$key]['content'],$args[1]);
                    break;
                case 2:
                default:
                    $this->checked[$key] = $this->v->$rule($this->items[$key]['content'],$args[1],$args[2]);
                    break;
            }
        }
    }
